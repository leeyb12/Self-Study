...
- 노트와 폴더는 사용자별 계층 구조를 이루며 이동·삭제 시 하위 데이터의 처리 규칙이 필요합니다.
- Markdown 원문은 편집 데이터이고 렌더링된 HTML은 표시 결과이므로 저장과 출력 단계를 구분합니다.
- 첨부파일은 메타데이터를 DB에 저장하고 실제 바이트는 파일 시스템에 저장해 역할을 나눕니다.
- 휴지통은 즉시 삭제 대신 삭제 시각이나 상태를 기록하는 소프트 삭제 방식으로 복구 가능성을 제공합니다.
- AI 기능은 현재 노트의 문맥을 추출해 모델에 전달하고 생성 결과를 사용자가 검토한 뒤 반영하도록 구성합니다.

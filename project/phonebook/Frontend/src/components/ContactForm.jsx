import { useState } from 'react'; // ⭐️ useEffect 제거
import {
...
export default function ContactForm({ initial, groups, onSubmit, onCancel }) {
  // ⭐️ useState 초기값 단계에서 initial 존재 여부를 판별하여 즉시 대입합니다.
  const [form, setForm] = useState(() => 
    initial
      ? { ...EMPTY, ...initial, groupId: initial.groupId ?? '' }
      : EMPTY
  );

  // 🚨 [삭제 완료] ESLint 에러를 유발하던 useEffect 구문은 완전히 제거되었습니다.
    ...
    <Dialog open onClose={onCancel} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers>
        <Stack spacing={2.5} sx={{ pt: 1 }}>
          ...
          <TextField
            label="지번주소" fullWidth
            value={form.addrJibun}
            InputProps={{ readOnly: true }}
            size="small"
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={7}>
              <TextField
                label="상세주소" fullWidth
                value={form.addrDetail} onChange={set('addrDetail')}
                placeholder="동, 호수 등 직접 입력"
              />
            </Grid>
            <Grid item xs={12} sm={5}>
              <TextField
                label="참고항목" fullWidth
                value={form.addrExtra}
                InputProps={{ readOnly: true }}
                size="small"
                sx={{ '& .MuiInputBase-input': { fontSize: 13, color: 'text.secondary' } }}
              />

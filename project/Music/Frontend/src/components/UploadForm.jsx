                ...
                <div style={modalOverlayStyle} onClick={() => !loading && setOpen(false)}>
                    ...
                    <div style={modalContentStyle} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            <h3 style={{ margin: 0, fontSize: '16px', color: '#1a1a1a' }}>음악 업로드</h3>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '4px' }}>
                                    <div style={coverPreviewBoxStyle} onClick={() => imageInputRef.current?.click()}>
                                        {imagePreview ? (
                                            ...
                                            <span style={{ color: '#f5f0e8', fontSize: '20px' }}>+</span>
                                        )}
                                    </div>
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        <button onClick={() => imageInputRef.current?.click()} style={smallBtnStyle}>변경</button>
                                        {imagePreview && <button onClick={() => { setImageFile(null); setImagePreview(null); }} style={{ ...smallBtnStyle, color: '#c0392b' }}>제거</button>}
                                    </div>
...
const modalContentStyle = {
    background: '#fff', width: '400px', padding: '24px', borderRadius: '8px',
    boxShadow: '0 10px 25px rgba(0,0,0,0.2)', border: '1px solid #ede8df'
};

const closeXBtnStyle = { background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: '#999', lineHeight: '1' };
const labelStyle = { fontSize: '11px', color: '#999', fontWeight: 'bold', display: 'block', marginBottom: '4px' };
const inputStyle = { padding: '10px', border: '1px solid #e0dbd2', borderRadius: '4px', fontSize: '13px', background: '#faf9f6', outline: 'none', width: '100%', boxSizing: 'border-box' };
const submitBtnStyle = { marginTop: '10px', padding: '12px', background: '#1a1a1a', color: '#f5f0e8', border: 'none', borderRadius: '4px', fontSize: '13px', cursor: 'pointer', width: '100%', fontWeight: 'bold' };
const smallBtnStyle = { padding: '4px 8px', background: '#fff', border: '1px solid #ddd', borderRadius: '2px', fontSize: '11px', color: '#666', cursor: 'pointer' };
const coverPreviewBoxStyle = { width: '48px', height: '48px', borderRadius: '4px', background: '#1a1a1a', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #ddd' };
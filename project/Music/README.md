...
- JWT는 로그인 상태를 서버 세션 없이 검증할 수 있게 하며 만료와 재로그인 흐름을 함께 고려합니다.
- 음원 파일과 커버 이미지는 파일 저장소에 두고 Oracle에는 곡 정보와 파일 경로를 저장합니다.
- 오디오 재생 상태는 현재 곡, 재생 위치, 볼륨과 반복 모드를 하나의 상태 흐름으로 관리합니다.
- WebSocket 채팅은 연결 상태, 구독 목적지와 메시지 형식을 클라이언트와 서버가 동일하게 사용해야 합니다.
- 게시글과 댓글은 작성자 관계를 통해 수정·삭제 권한을 검증합니다.

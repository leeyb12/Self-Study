...
function OnlineGomoku() {
  const [playerId] = useState(makeId)
  const [roomInput, setRoomInput] = useState('')
  const [room, setRoom] = useState(null)
  const { state, connected, error, publish } = useStompRoom('omok', room, playerId)
  ...
  if (!room) {
    return (
      <div className="game">
        ...
        <p className="game-info">같은 코드를 입력한 두 사람이 오목으로 대결합니다.</p>
  ...
  const board = state?.board ?? Array(SIZE * SIZE).fill(null)

  return (
    <div className="game">
      <p className="game-message">{status}</p>
      {mySymbol && <p className="game-info">내 돌: {STONE[mySymbol]}</p>}
